...
class TestUserManagement(TestCase):
    ...
    def test_user_with__first_name_login(self):
        ...
        # главная страница после логина пользователя
        response = self.client.get('/')
        self.assertEqual(response.status_code, 200)
    ...
    def test_superuser_login(self):
        ...
        # проверяем главную страницу после логина суперпользователя
        response = self.client.get('/')
        self.assertEqual(response.status_code, 200)
        self.assertIn('админка', response.content.decode())

    def test_basket_login_redirect(self):
        # проверка переадресации на страницу логина если пользователь не авторизовался
    ...
    def test_user_logout(self):
        ...
        # главаная страница после разлогина
        response = self.client.get('/')
        self.assertEqual(response.status_code, 200)
        self.assertTrue(response.context['user'].is_anonymous)
    ...
    def test_user_register(self):
        ...
        activation_url = f"{settings.DOMAIN_NAME}/auth/verify_link/{new_user_data['email']}/{new_user.activation_key}/"
    ...
    # проверка срабатывания ограничения пользователей по возрасту при регистрации
